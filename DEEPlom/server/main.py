...
def send_restore_pass(username:str):
    try:
        ...
        a = smtpObj.starttls()
        print(a)
        a = smtpObj.login(os.getenv("EMAIL_NAME"),os.getenv("EMAIL_PASS"))
        print(a)
        a = smtpObj.sendmail(os.getenv("EMAIL_NAME"),user.email,msg)
        print(a)
        smtpObj.close()
        db.add(restore)
        db.commit()
...
def buy(fullprice:float,credentials: JwtAuthorizationCredentials = Security(access_security)):
...
async def getRecommendations(user_id: Optional[int]):
    ...
    if user_id:
        ...
        result = recommend_games(user_id, model, user_id_map, game_id_map, df, 4)
        ...
        top_3_games = game_counter.most_common(4)
